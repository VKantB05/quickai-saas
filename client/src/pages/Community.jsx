  ...
  const imageLikeToggle = async (id)=>{
    try {
      ...
      if(data.success){
        ...
        await fetchCreations() // Refresh list to show updated count
      }else{
        toast.error(data.message);
      }
    } catch (error) {
      ...
      <div className='bg-white h-full w-full rounded-xl overflow-y-scroll'>
        {creations.map((creations, index) => (
          <div key={index} className='relative group inline-block pl-3 pt-3 w-full sm:max-w-1/2 lg:max-w-1/3 '>
            <img src={creations.content} alt='' className='w-full h-full 
            ...
            rounded-lg'>
              ...
              <div className='flex gap-1 items-center'>
                ...
                <Heart onClick={() => imageLikeToggle(creations.id)} className={`min-w-5 h-5 hover:scale-110 cursor-pointer 
                  ${creations.likes.includes(user.id) ? 'fill-red-500 text-red-600' :
                    'text-white'}`} />
              </div>
            </div>
          </div>
